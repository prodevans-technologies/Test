...
class Toolbar extends widgets_1.Widget {
    ...
    _insertRelative(at, offset, name, widget) {
        let nameWithIndex = algorithm_1.map(this.names(), (name, i) => {
            return { name: name, index: i };
        });
        let target = algorithm_1.find(nameWithIndex, x => x.name === at);
        if (target) {
...
(function (Toolbar) {
    ...
    function createKernelNameItem(session) {
        const el = vdom_1.ReactWidget.create(React.createElement(Private.KernelNameComponent, { session: session }));
...
function ToolbarButtonComponent(props) {
    ...
    return (React.createElement(ui_components_1.Button, { className: props.className
...
exports.addToolbarButtonClass = addToolbarButtonClass;
/**
 * Phosphor Widget version of static ToolbarButtonComponent.
 */
class ToolbarButton extends vdom_1.ReactWidget {
    /**
...
function CommandToolbarButtonComponent(props) {
    return (React.createElement(vdom_1.UseSignal, { signal: props.commands.commandChanged, shouldUpdate: (sender, args) => (args.id === props.id && args.type === 'changed') ||
...
exports.addCommandToolbarButtonClass = addCommandToolbarButtonClass;
/**
 * Phosphor Widget version of CommandToolbarButtonComponent.
 */
class CommandToolbarButton extends vdom_1.ReactWidget {
    /**
...
(function (Private) {
    ...
    function KernelNameComponent(props) {
        return (React.createElement(vdom_1.UseSignal, { signal: props.session.kernelChanged, initialSender: props.session }, session => (React.createElement(ToolbarButtonComponent, { className: TOOLBAR_KERNEL_NAME_CLASS, onClick: props.session.selectKernel.bind(props.session), tooltip: 'Switch kernel', label: session.kernelDisplayName }))));
    }
